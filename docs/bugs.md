...
### BUG-036: pkill -f DangerDetector pattern misses flag-order variants; test target never wired up
- **Description**: (1) DangerDetector's `pkill -f` pattern (`pkill\s+-f`) only matched when `-f` immediately followed `pkill`, so the very common `pkill -9 -f <name>` (signal flag before `-f`) went undetected. (2) Separately, `Tests/ClaudeGateTests/` had existed since project start but `Package.swift` never declared a `.testTarget`, so `swift test` had no target to discover — the placeholder test (`testPlaceholder`) had never actually executed, locally or in CI (`build.yml` only ran `swift build`).
- **Root Cause**: (1) DangerDetector.swift:18 — regex anchored `-f` directly after `pkill`, no allowance for preceding flags like `-9`. (2) Package.swift never had a `.testTarget` entry despite the Tests directory existing.
- **Status**: FIXED
- **Fix**: (1) Regex widened to `pkill(\s+-[^\s]+)*\s+(-f|--full)(\s|$)`, matching `-f`/`--full` as a token preceded by any number of other flag tokens, in either order (`pkill -f -9`, `pkill -9 -f`), while still requiring an exact `-f`/`--full` token (avoids false positives on things like `-farfetched`). Message now names the alternative: `pgrep <name>`, then `kill <pid>`. Deliberately left bare `pkill <name>` and `killall <name>` unflagged — they're name-only matches, lower risk than `-f`'s "match against full argv" semantics, and flagging them risks alert fatigue. (2) Added `.testTarget` to Package.swift and `DangerDetectorTests.swift` covering both the fix and the deliberate non-matches; CI now runs `swift test` before `swift build -c release`.
- **Note**: `swift test` requires full Xcode (XCTest.framework); this dev machine only has Command Line Tools, so the new suite could only be verified here by replicating the exact regex against Foundation's regex engine directly (all cases passed). It will run for real on GitHub Actions' `macos-14` runner, which has Xcode installed.
